...
- Once the script finishes, you can validate that the MEU objects were successfully created and manually re-enable the Azure AD Connect sync sycle using the following cmdled: `Set-ADSyncScheduler -SyncCycleEnabled 1`

- All MEU objects will be created using the UPN suffix that you passed through `-UPNSuffix` param. If you need to create MEUs with different UPN suffix, you will need to segment the MEU creation based on the UPN suffix, it means run the script for each UPN suffix.
...
4 - Run the script `Cross-Tenant-Migration-Attribute-Export.ps1` to dump the source mailboxes and validate yourself that the CSV generated by the script contains only mailboxes filtered by the custom attribute.
...
| BypassAutoExpandingArchiveCheck         | Switch to bypass the Auto-Expanding archive mailboxes check.¹ | Optional |
...
```Powershell
.\Cross-Tenant-Migration-Attribute-Export.ps1 -AdminUPN admin@contoso.com -CustomAttributeNumber 1 -CustomAttributeValue T2T -DomainMappingCSV sourcetarget.csv -LocalMachineIsNotExchange -ExchangeHostname ExchangeServerMachine
```
...
| Password                                | Choose a password for all new MEU objects. If no password is chosen, the script will define "?r4mdon-p@ss0rd!" as password. | Optional |
| ResetPassword                           | Require password change on first user access. | Optional |
| OU                                      | Create MEU objects in a specific Organization Unit. Valid values are name, Canonical name, Distinguished name (DN) or GUID. | Optional |
...
```Powershell
.\Cross-Tenant-Migration-Attribute-Import.ps1 -UPNSuffix contoso.com -Password "P@$$w04d@098" -ResetPassword $True -OU Contoso 
```
