...
Param(
    ...
    the script will define '?r4mdon-p@ss0rd!' as password")]
    [string]$Password,
...
{ 
    ...
    Write-Progress -Activity "Creating MEU objects and importing attributes from CSV" -Status "Working on $($user.DisplayName)" -PercentComplete ($p/$ImportUserList.Count*100)
    ...
    # Convert legacyDN to X500. As we used ";" as delimiter in EmailAddress
...
Write-Host "$(Get-Date) - The import was finished. Please confirm that all users are correctly created before enable the Azure AD Sync Cycle. 
You can enable using the following cmdlet: Set-ADSyncScheduler -SyncCycleEnabled 1 " -ForegroundColor Green
